...
class KTBMonitors extends Component {
  ...
  componentDidMount() {
    //fetch('http://localhost:52519/api/OrgPosition')
    //fetch('http://localhost:8080')
    fetch('http://localhost:8080/getDepartment')
    .then(res => res.json())
    .then((data) => {
      //this.setState({ org_positions: data })
      //console.log(this.state.org_positions);
      this.setState({ org_departments: data })
      console.log(this.state.org_departments);
    })
    .catch((error) => {
  ...
  render() {
    return (
      <div className="animated fadeIn">
              {/* <OrgPosition orgposition={this.state.org_positions} /> */}
              <OrgDepartment orgdepartment={this.state.org_departments} />
        {/* 
        <Row>
          <Col sm="12" xl="6">
            <Card>
                ...
                <Row>
                  <Col xs="4">
                    <ListGroup id="list-tab" role="tablist">
                      <ListGroupItem onClick={() => this.toggle(0)} action active={this.state.activeTab === 0} >Home1</ListGroupItem>
